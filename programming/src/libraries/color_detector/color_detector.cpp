...
Mat detectColorHSV(string color, Mat image ){
    ...
    if(color == "red"){
        Mat thres1;
        Mat thres2;
        inRange(HSV, Scalar(0, 40, LowV), Scalar(9, HighS, HighV), thres1);
        inRange(HSV, Scalar(160, 40, LowV), Scalar(179, HighS, HighV),thres2); //Threshold the image
        imgThresholded=thres1+thres2;
    }else{
        if(color=="green") {LowH=25; HighH=72;}
        else if(color=="blue") {LowH=90; HighH=140;}
        else if(color=="yellow") {LowH=10; HighH=40;LowS=45;}
        else report(ERROR, "Bad color detection");
...
Mat detectColorHSV(int color, Mat image ){
    string col;
    if(color==COLOR_RED) col="red";
    else if(color==COLOR_GREEN) col="green";
    else if(color==COLOR_BLUE) col="blue";
    else if(color==COLOR_YELLOW) col="yellow";

    else report(ERROR, "Bad color detection");

    return detectColorHSV(col, image );
}

//////////////////////////////////////////////////////7
vector<Point> getTargetHSV(std::string color, Mat image){

    Mat thresh=detectColorHSV(color,image);
...
vector<Point> getTargetHSV(int color, Mat image){
